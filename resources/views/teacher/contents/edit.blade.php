    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route(Auth::user()->role . '.courses.contents.update', [$course, $content]) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="body" name="body" rows="8" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>{{ old('body', $content->body) }}</textarea>
                        <x-input-error :messages="$errors->get('body')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="order_sequence" :value="__('Urutan Materi (Angka)')" />
                        <x-text-input id="order_sequence" class="block mt-1 w-full" type="number" name="order_sequence" :value="old('order_sequence', $content->order_sequence)" required />
