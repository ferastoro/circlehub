        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form method="POST" action="{{ route(Auth::user()->role . '.courses.update', $course) }}">
                    ...
                    {{-- Hanya Admin yang bisa memilih/mengganti Teacher --}}
                    ...
                    <div class="mt-4">
                        <x-input-label for="status" :value="__('Status Course')" />
                        <select id="status" name="status" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                            ...
                            <option value="inactive" {{ old('status', $course->status) == 'inactive' ? 'selected' : '' }}>Non-Aktif</option>
                        </select>
                    ...
                    <div class="grid grid-cols-2 gap-4 mt-4">
                        <div>
                            ...
                            <x-text-input id="start_date" class="block mt-1 w-full" type="date" name="start_date" :value="old('start_date', $course->start_date)" required />
                            <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="end_date" :value="__('Tanggal Selesai')" />
                            <x-text-input id="end_date" class="block mt-1 w-full" type="date" name="end_date" :value="old('end_date', $course->end_date)" required />
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="description" name="description" rows="4" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>{{ old('description', $course->description) }}</textarea>
